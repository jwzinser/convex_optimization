...
%%
cvx_begin
variables st(T) theta(n,T)
minimize (T+ones(1,T)*st+st'*eye(T)*st)
subject to
-R<=st(2:T)-st(1:T-1)<=R
Smin*ones(T,1)<=st<=Smax*ones(T,1)
ones(1,n)*theta==ones(1,T)
theta>=zeros(n,T)
theta*st>=W
for i=1:n
    if A(i)>1
        theta(i,1:A(i)-1)==zeros(1,A(i)-1)
    end
    if D(i)<T
        theta(i,D(i)+1:T)==zeros(1,T-D(i)-1)
    end
end
%%
